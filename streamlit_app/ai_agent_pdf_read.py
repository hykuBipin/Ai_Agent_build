...
if uploaded_pdf is not None:
    ...
    if not pdf_text.strip():
        ocr_used = True
        st.info("No text found with normal extraction. Attempting OCR...")
    ...
    if pdf_text.strip():
        ...
        if ocr_used:
            st.info("Text extracted using OCR. Check the preview below.")
        st.text_area("Extracted PDF Text Preview", pdf_text, height=300)
    else:
        st.error("Failed to extract meaningful text from PDF, even with OCR.")
